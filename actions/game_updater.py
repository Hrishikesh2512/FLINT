...
def _cancel_scheduled_update() -> str:
    result = subprocess.run(["schtasks", "/Delete", "/TN", "FLINT_GameUpdater", "/F"],
                            capture_output=True, text=True)
    return "Scheduled update cancelled." if result.returncode == 0 else "No scheduled update found."


def _get_schedule_status() -> str:
    result = subprocess.run(["schtasks", "/Query", "/TN", "FLINT_GameUpdater", "/FO", "LIST"],
                            capture_output=True, text=True)
    if result.returncode != 0:
...
@tool(
    ...
    parameters={'action': {'type': 'STRING', 'description': 'update | install | list | download_status | schedule | cancel_schedule | schedule_status (default: update)'}, 'platform': {'type': 'STRING', 'description': 'steam | epic | both (default: both)'}, 'game_name': {'type': 'STRING', 'description': 'Game name (partial match supported)'}, 'app_id': {'type': 'STRING', 'description': 'Steam AppID for install (optional)'}, 'hour': {'type': 'INTEGER', 'description': 'Hour for scheduled update 0-23 (default: 3)'}, 'minute': {'type': 'INTEGER', 'description': 'Minute for scheduled update 0-59 (default: 0)'}, 'shutdown_when_done': {'type': 'BOOLEAN', 'description': 'Shut down PC when download finishes'}},
    required=[],
)
def _tool_game_updater(args, ctx):
