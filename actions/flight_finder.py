...
@tool(
    ...
    parameters={'origin': {'type': 'STRING', 'description': 'Departure city or airport code'}, 'destination': {'type': 'STRING', 'description': 'Arrival city or airport code'}, 'date': {'type': 'STRING', 'description': 'Departure date (any format)'}, 'return_date': {'type': 'STRING', 'description': 'Return date for round trips'}, 'passengers': {'type': 'INTEGER', 'description': 'Number of passengers (default: 1)'}, 'cabin': {'type': 'STRING', 'description': 'economy | premium | business | first'}, 'save': {'type': 'BOOLEAN', 'description': 'Save results to Notepad'}},
    required=['origin', 'destination', 'date'],
)
def _tool_flight_finder(args, ctx):
    return flight_finder(parameters=args, player=ctx.player) or "Done."
