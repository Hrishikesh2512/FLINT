"""Small persistent stores the assistant keeps: reminders, notes, lists, people.

Every one of these is a JSON document under a lock, written atomically, and
none of them knows what body it is running in. A shopping list is a shopping
list on a Pi, a phone or a laptop, and a person's phone number does not change
because a different device wrote it down — so they live in the shared core and
are synced between devices rather than being re-implemented per body.

Reminders differ from timers, and the distinction is the reason they are here
rather than in RAM: timers are relative countdowns held in memory
(`TimerBoard`), while reminders are absolute wall-clock ("tomorrow 8am") that
must outlive a reboot.
"""
...
class ConnectionStore(_JsonStore):
    ...
    def save(self, name: str, phone: str = "", nickname: str = "",
        ...
        with self._lock:
            ...
            # When this record last changed. Sync needs it to decide which of
            # two devices' versions of a person is newer; every field here
            # merges rather than replaces, so without a stamp two devices that
            # each learned something different about the same person have no
            # way to agree on an order.
            rec["ts"] = time.time()
            # And this edit belongs to whichever device is making it. A record
            # that arrived by sync carries the device that wrote it, which is
            # right until someone edits it *here* — leave the old attribution
            # on and the sync layer reads our own edit as that device's change
            # coming back round, skips it, and the edit never leaves this box.
            rec.pop("device", None)
            data[key] = rec
            self._save(data)
        return rec
